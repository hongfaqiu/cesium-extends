...
import type { DataSource, Entity ,
  Billboard,
  Label,
  PointPrimitive} from 'cesium';
...
export const dataSourceRender = async (dataSource: DataSource, style: EntityStyle) => {
...
export const renderGeoJson = async (
  ...
  const entityStyle: EntityStyle = await RenderConfig2Style[type](data, style as any);
...
const clusterEvent = (clusteredEntities: Entity[], cluster: {
  billboard: Billboard;
  label: Label;
  ...
  const { alpha = 1, scale = 1, colorBar = [
    '#313695',
    '#4575b4',
    '#74add1',
    '#abd9e9',
  ...
  if (length <= maxNum) {
    ...
    cluster.billboard.image = (new BillBuilder().fromText(num >= 10 ? `${num}+` : String(length), Color.fromCssColorString(colorBar[Math.floor(length / maxNum * colorBar.length)] ?? colorBar[colorBar.length - 1]), 80 + length / maxNum * 100)).toDataURL();
  } else {
    cluster.billboard.image = (new BillBuilder().fromText(`${length}`, Color.fromCssColorString(colorBar[colorBar.length - 1]), 180)).toDataURL();
  }
...
function dataSourceCluster(
  ...
  if (enable) dataSource.clustering.clusterEvent.addEventListener((clusteredEntities, cluster) => clusterEvent(clusteredEntities, cluster, options));
}
